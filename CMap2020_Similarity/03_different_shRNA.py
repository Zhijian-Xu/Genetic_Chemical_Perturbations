...
for name, group in grouped:
    ...
    for i in range(len(samples)):
        for j in range(i + 1, len(samples)):
            ...
            if group.iloc[i, 1] != group.iloc[j, 1]:
                if sample1 in df2.index:
                    ...
                    jaccard_similarity = round(similar_elements / len(expr1), 4) 
                    ...
                    results.append([sample1, sample2, jaccard_similarity, pearson_similarity, spearman_similarity, cosine_similarity_value])


result_df = pd.DataFrame(results, columns=['Sample1', 'Sample2', 'Jaccard', 'Pearson', 'Spearman', 'Cosine_diffshRNA'])

result_df.to_csv('./result/CMap2020_differentshRNA.txt', index=False, sep='\t')
