...
def calculate_cosine_similarity(time_fixed, output_filename):
    ...
    filtered_df1 = df1[(df1[6] == str(time_fixed)) & (df1[8].isin(['trt_sh', 'trt_sh.cgs']))]
    ...
    result_df = pd.DataFrame(results, columns=['Sample1', 'Sample2', f'GSE70138-Knockdown_{time_fixed}h'])
    result_df.to_csv(output_filename, index=False, sep='\t')
    print(f"Saved results to {output_filename}")

calculate_cosine_similarity(96, './result/CMap2020_Knockdown_96h_cosine.txt')
calculate_cosine_similarity(120, './result/CMap2020_Knockdown_120h_cosine.txt')
calculate_cosine_similarity(144, './result/CMap2020_Knockdown_144h_cosine.txt')
